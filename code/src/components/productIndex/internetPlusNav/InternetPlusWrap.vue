        ...
        <div class="grid-content">
          <div class="side-menu-main">
            <ul class="side-out" :style="{height:winHeight + 'px'}">
               ...
               <li v-for="(item,index) in sideMenuData" :class="index == activeIndex ?'side-out-li active':'side-out-li'"   @click="getChildListAndWordList(item.Id,index,item.Title)">
                  ...
                  :style="{width: 0.15 * winWidth +'px'}"
                   id="divId">
                     <li v-for="(ele,i) in item.children" class="side-in-li" @click="getChildListAndWordList(ele.Id)">
      ...
      <el-col :span="21">
        <div class="input-content">
          <h3 class="knowledge-contet fl">知识导航
              <i class="el-icon-arrow-right" v-show="oneTitle"></i>
              <span style="color:#00aeed"> {{oneTitle}}</span>
          </h3>
...
export default {
  ...
  methods:{
    ...
    //   this.activeChildTitle = item.Title;
    // },
     /**
     ...
     * @return   {[type]}   [description]
     */
    getChildListAndWordList(id,index,title) {
      if(id){
        this.oneTitle = title;
      }
       this.activeIndex = index || 0;
      ...
      this.$http.get("/Category/ListByGroupName", {
        ...
        .then((res) => {
          ...
          if (res.data.Code == 200) {
            ...
            this.activeChildTitle = this.sideMenuData[0].Title;
