  ...
  methods:{
    ...
    getInitChildList(){
      ...
      this.$http.get("/Category/ChildList", {
          params: {
            ...
            type: this.extendData.LearningGroupName,
            isSelf:false
          }
        })
        .then((res) => {
    ...
    jsmindNodeClick(e) {
      ...
      if(e.target.nodeName != 'DIV') {
         //点击的是展开按钮
         if(e.target.nodeName == 'JMEXPANDER') return;
         ...
         if(this.jsMind.get_selected_node()) {
            ...
            // }else {
              ...
              // console.log(nodesInfo.data.data.Id)
              if(nodesInfo.data.data.Level) {
                 this.goToUrl('/knowledgeList',{
                   query:[{
                    id:nodesInfo.data.data.Id
                   }],
                   winType:'资源',
                   winTitle:nodesInfo.data.data.Title + '关联知识元列表',
                   isIndForm:true,
                 });
              }else {
                //如果是知识元节点，就该去看详情了
