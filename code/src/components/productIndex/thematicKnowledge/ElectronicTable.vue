  ...
  <el-table
      ...
      <el-table-column type="expand" >
        <template slot-scope="props" >
            <div  style="width:70%;line-height:20px;margin:10px auto;padding:10px 15px;outline: 1px dashed #ddd;">
                    {{props.row.Abstracts?'内容摘要：'+props.row.Abstracts:'内容摘要：暂无'}}
            </div>
        </template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column
        ...
        <template slot-scope="props">
         <el-button
           ...
           @click.stop="goOtherPage('/eStandard',props.row.Id,props.row.Title)">查看详情
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
   ...
   */
     getList() {
        ...
        this.$http.get("/Content/Search", {
          params: {
            categoryIds: this.oneChildId ? this.oneChildId : 1,
            objectTypes: 103,
            ps: this.pageSize,
            cp: this.currentPage,
          }
  ...
  watch:{
    oneChildId:function(val,oldVal){
      this.currentPage = 1;
      this.getList();
    },
