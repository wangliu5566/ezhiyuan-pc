...
// ============= data provider =============================================
    ...
    jm.view_provider.prototype={
        ...
        create_node_element:function(node,parent_node){
            ...
            //修改源码，当层级为3，展示数量
            if(node.data.data.Level == 3) {
                var d_n = $c('span');
                d_n.innerText = node.data.data.ExplicitWordList.length;
                
                //左侧 -1  右侧 1
                if(node.direction == -1) {
                  d_n.className = 'expander-left';
                }else {
                  d_n.className = 'expander-right';
                }
 
                d_n.addEventListener('click', function(e) {
                   e.preventDefault();
                   e.stopPropagation();
                }, false)

                d.appendChild(d_n);
            }else if(!node.data.data.Level){
                //根据边框区分知识元
                d.style.border = '2px solid #2aa';
            }
